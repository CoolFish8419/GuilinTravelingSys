...
export default {
  ...
  mounted() {
    const throttledtleScroll = this.throttle(this.scroll, 200);
    window.addEventListener('scroll', throttledtleScroll);
  },
  methods: {
    getData(val) {
    ...
    async scroll() {                //检测滚轮
      ...
      if (this.$refs.option) {
        console.log('rect ready')
        let rect = this.$refs.option.getBoundingClientRect();
        this.top = rect.top
    ...
    throttle(func, delay) {
      ...
      return function (...args) {
        ...
        const timeSinceLatsExec = currentTime - lastExecTime;

        //如果距离上次执行的时间大于delay则再次执行
        if (timeSinceLatsExec > delay) {
  ...
  <div class="content">
    ...
    <div class="option" ref="option">
      ...
      <div class="option_content">
        <div v-for="item in list" :key="item" class="item" @click="goDetails(item.id, 'hotel')">
          <a-image width="210px" :src="item.image" height="158px" fit="cover" style="margin:10px;" :title="item.score"
            :preview="preview" />
